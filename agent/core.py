...
TOOL_CAPABLE_MODELS = {"qwen3:8b", "qwen2.5-coder:7b", "qwen2.5:7b", "llama3.1:8b", "llama3.3:70b", "gemma4:12b"}
...
TOOL_PATTERNS = {
    ...
    "edit_file": [
        r"\b(edit|modify|change|update|replace|fix|patch|corregir|modificar|cambiar|actualizar)\b.*\b(file|code|script|archivo)\b",
        r"\b(in|en|file|archivo)\b.*\b(replace|cambiar|replace)\b.*\b(with|con)\b",
    ],
    "grep_code": [
...
def _extract_tool_params(tool_name: str, message: str) -> dict:
    ...
    if tool_name == "edit_file":
        m = re.search(r"(?:edit|modify|change|update|replace|fix|patch)\s+(?:in\s+)?(\S+\.\w+)", message, re.IGNORECASE)
        path = m.group(1) if m else ""
        m2 = re.search(r"(?:replace|cambiar)\s+['\"](.+?)['\"]\s+(?:with|con)\s+['\"](.+?)['\"]", message, re.IGNORECASE)
        if m2:
            return {"path": path, "old_string": m2.group(1), "new_string": m2.group(2)}
...
class ZICoreAgent:
    ...
    def _ollama_chat(self, message: str, model: str = None,
        ...
        try:
            ...
            _temperature, _top_p, _max_tokens = 0.7, 0.9, 2048

        payload = {
            "model": model,
            "messages": messages,
            "stream": False,
            "options": {"temperature": _temperature, "top_p": _top_p, "num_predict": _max_tokens},
        }

        use_tools = model in TOOL_CAPABLE_MODELS or any(model.startswith(m) for m in ["qwen3:", "qwen2.5-coder:", "llama3."])
        if use_tools:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=120) as resp:
                ...
                # Handle native tool calls from qwen3/llama3 models
                if use_tools and msg.get("tool_calls"):
                    ...
                    for tc in msg["tool_calls"]:
                        ...
                        logger.info(f"Native tool call: {fname}({fargs})")
                        try:
                    ...
                    if tool_context_parts:
                        ...
                        with urllib.request.urlopen(req2, timeout=120) as resp2:
                            ...
                            return result2.get("message", {}).get("content", "")
    ...
    def _detect_intent(self, message: str) -> str:
        ...
        if any(w in msg for w in ["brightness", "brillo", "dim", "bright", "oscurecer", "aclarar"]):
        ...
        if any(w in msg for w in ["edit file", "modify file", "change file", "replace in", "fix file",
                                    ...
                                    "edita archivo", "modifica archivo", "cambia archivo"]):
            return "edit_file"
        ...
        if any(w in msg for w in ["find files", "list files matching", "glob", "all files with",
                                    ...
                                    "todos los archivos", "archivos con"]):
            return "glob_files"
