...
# ║     macOS Installer — zicore.space              ║
# ╚══════════════════════════════════════════════════╝
#
# Usage:
#   curl -sL https://zcs.zicore.space/installers/install_zicore_mac.sh | bash
#   or download and run:
#   chmod +x install_zicore_mac.sh && ./install_zicore_mac.sh
#
# Requires: macOS 12+ (Monterey or later)
# Optional: Homebrew (auto-installed if missing)
...
banner() {
    ...
    echo "  ║     macOS Installer — zicore.space              ║"
    echo "  ╚══════════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
brew install git curl wget 2>/dev/null || true
...
if command -v node &>/dev/null; then
    ...
    if [ "$NODE_VER" -ge 20 ]; then
        ...
        brew link --overwrite node@20 2>/dev/null
    fi
else
...
if [ "$INSTALL_ML" = "y" ] || [ "$INSTALL_ML" = "Y" ]; then
    $PYTHON -m pip install --quiet torch 2>/dev/null
