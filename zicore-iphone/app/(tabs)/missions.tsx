...
export default function MissionsScreen() {
  ...
  const renderMission = ({ item }: { item: Mission }) => {
    ...
    return (
      <TouchableOpacity style={styles.card} activeOpacity={0.7} onPress={() => haptic.light()}>
        <View style={styles.cardHeader}>
          <View style={[styles.phaseIcon, { borderColor: phaseColor }]}>
            <Ionicons
              name={item.phase === 'active' ? 'rocket' : item.phase === 'complete' ? 'checkmark' : 'document-text'}
              size={16}
              color={phaseColor}
            />
          </View>
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.addBtn} onPress={() => haptic.medium()}>
