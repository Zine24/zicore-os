...
class VRStreamRenderer:
    ...
    def _draw_stars(self, draw, w, h, scene_type, frame_num, camera):
        ...
        t = frame_num * 0.02
        for sx, sy, brightness, size in self.stars_cache[key]:
...
class VRStreamManager:
    ...
    def update_camera(self, session_id: str, **kwargs) -> bool:
        """Update camera state for a session. Marks as client-controlled."""
        with self._lock:
            ...
            for key, val in kwargs.items():
                if hasattr(cam, key):
                    setattr(cam, key, val)
            session["client_controlled"] = True
            session["last_client_input"] = time.time()
            return True
    ...
    def render_frame(self, session_id: str) -> Optional[Dict]:
        ...
        # Auto-rotate only when no client has sent camera input for 3+ seconds
        if frame_num > 0:
            now = time.time()
            last_input = session.get("last_client_input", 0)
            if not session.get("client_controlled") or (now - last_input) > 3.0:
                cam.yaw += 0.3  # Slow auto-rotate when idle
