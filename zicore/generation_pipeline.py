...
class GenerationPipeline:
    ...
    def generate_3d(self, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        ai3d = self._get_ai3d()
        if ai3d:
            # auto chain first — the "single generator that always works"
            try:
                r = ai3d.generate(engine_key="auto", prompt=prompt)
                if r and getattr(r, "success", False) and getattr(r, "file_path", ""):
                    result = {
                        "status": "ok",
                        "file_stl": getattr(r, "file_path", ""),
                        "engine": f"ai3d_{getattr(r, 'metadata', {}).get('auto_chain', 'auto')}",
                ...
                logger.warning(f"AI3D auto failed: {e}")
